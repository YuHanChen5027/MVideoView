...
public class MyVideoView extends SurfaceView implements IMediaPlayer {
     ...
     * @return 音轨数
     */
    private List<Integer> getVideoTrackNum() {
        List<Integer> count = new ArrayList<>();
        if (mMediaPlayer != null && mMediaPlayer.getTrackInfo().length >= 2) {
            for (int i = 0; i < mMediaPlayer.getTrackInfo().length; i++) {
                if (mMediaPlayer.getTrackInfo()[i].getTrackType() == MediaPlayer.TrackInfo.MEDIA_TRACK_TYPE_AUDIO) {
                    count.add(i);
                }
            }
        }
